...
// ── CMD mode store ────────────────────────────────────────────────────────────
// Keyed by chatJid → array of stored items (max 20 per chat)
const _cmdStore = new Map();
const CMD_MAX   = 20;

function _cmdPush(chatJid, item) {
  if (!_cmdStore.has(chatJid)) _cmdStore.set(chatJid, []);
  const arr = _cmdStore.get(chatJid);
  arr.push(item);
  if (arr.length > CMD_MAX) arr.shift();
}

function getCmdDeleted(chatJid) {
  return (_cmdStore.get(chatJid) || []).slice();
}

function clearCmdDeleted(chatJid) {
  _cmdStore.delete(chatJid);
}
...
module.exports.getCmdDeleted   = getCmdDeleted;
module.exports.clearCmdDeleted = clearCmdDeleted;
...
async function _flushBatch(batch, sock, settings, mediaCache, ownerJid, _timeStr, _dateStr) {
  ...
  // ── CMD mode: silently store, no automatic notification ───────────────────
  if (mode === 'cmd') {
    for (const item of items) {
      _cmdPush(from, { ...item, from, isGroup });
    }
    return;
  }
...
async function _sendRecovered(sock, destJid, original, deletedId, mediaCache, header, mentions) {
  try {
    // Unwrap ephemeral / viewonce wrappers so we get the real media type
    const origMsg =
      original.message?.ephemeralMessage?.message ||
      original.message?.viewOnceMessage?.message ||
      original.message?.viewOnceMessageV2?.message?.viewOnceMessage?.message ||
      original.message || {};
    const origType = Object.keys(origMsg)[0];
    const text     = origMsg.conversation || origMsg.extendedTextMessage?.text;
    ...
    if (!mediaBuf) {
      // Use reuploadRequest so Baileys can fetch a fresh CDN URL if the original expired
