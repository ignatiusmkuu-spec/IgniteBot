  ...
  try {
    switch (cmd) {
      ...
      case "menuv": {
        ...
        // Song plays first so user hears it before the menu visual appears
        const sendSongFirst = async () => {
          if (menuSong) {
            await sock.sendMessage(from, {
              audio:    menuSong,
              mimetype: "audio/mpeg",
        ...
        if (menuType === "image") {
          ...
          // ── Video mode (default) ──────────────────────────────────────
          if (menuVideo) {
            // Song first → then GIF video with menu caption
            await sendSongFirst();
            await sock.sendMessage(from, {
              video:       menuVideo,
              caption:     fullCaption,
