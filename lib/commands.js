...
function buildMenu(p) {
  ...
  return (
    ...
    `┗━━━━━━━━━━━━━━━━━━━━━━┛\n\n` +
    `🕐 *${timeStr}*  •  📅 *${dateStr}*\n` +
    `⚡ Prefix: *${p}*  •  Prefixless: ${pl ? "✅ ON" : "❌ OFF"}\n\n` +
    `_Reply with a section number or just use commands_`
  );
}
...
function buildMenuSections(p) {
  ...
  return [
    ...
    [
      ...
      `  ℹ️ *GENERAL*\n` +
      `${LINE}\n` +
      `› ${c("menu")} / ${c("help")} — Show this menu\n` +
      `› ${c("alive")} — Check if bot is online\n` +
      `› ${c("ping")} — Response latency & uptime\n` +
      `› ${c("time")} — Current date & time\n` +
      `› ${c("stats")} — Bot usage analytics\n` +
      `› ${c("uptime")} — RAM & uptime info`,

      `${LINE}\n` +
      `  ⚙️ *SETTINGS*\n` +
      `${LINE}\n` +
      `› ${c("botsettings")} — View all bot settings\n` +
      `› ${c("features")} — View all feature toggles\n` +
      `› ${c("feature [name] on/off")} — Toggle a feature\n` +
      `› ${c("lang [code]")} — Set language (e.g. fr, es)\n` +
      `› ${c("setprefix [char]")} — Change command prefix\n` +
      `› ${c("mode public/private/group")} — Set access mode`,
    ],
    ...
    [
      ...
      `  🤖 *AI & SMART*\n` +
      `${LINE}\n` +
      `› ${c("ai [message]")} — Chat with AI\n` +
      `› ${c("ask [question]")} — Quick Q&A\n` +
      `› ${c("imagine [prompt]")} — Generate AI image\n` +
      `› ${c("tts [text]")} — Text to voice note\n` +
      `› ${c("summarize [text]")} — Summarise long text\n` +
      `› ${c("clearchat")} — Clear your AI history`,

      `${LINE}\n` +
      `  🔍 *SEARCH & INFO*\n` +
      `${LINE}\n` +
      `› ${c("weather [city]")} — Live weather report\n` +
      `› ${c("wiki [topic]")} — Wikipedia summary\n` +
      `› ${c("define [word]")} — Dictionary definition\n` +
      `› ${c("tr [lang] [text]")} — Translate text\n` +
      `› ${c("langs")} — List all translate codes`,
    ],

    // ── Section 3: Fun & Text Tools ────────────────────────────────────
    [
      `${LINE}\n` +
      `  🎮 *FUN & GAMES*\n` +
      `${LINE}\n` +
      `› ${c("8ball [question]")} — Magic 8-ball\n` +
      `› ${c("fact")} — Random fun fact\n` +
      `› ${c("flip")} — Flip a coin\n` +
      `› ${c("joke")} — Random joke\n` +
      `› ${c("quote")} — Inspirational quote\n` +
      `› ${c("roll [max]")} — Roll a dice`,

      `${LINE}\n` +
      `  🔠 *TEXT TOOLS*\n` +
      `${LINE}\n` +
      `› ${c("aesthetic [text]")} — Ａｅｓｔｈｅｔｉｃ text\n` +
      `› ${c("bold [text]")} — 𝗕𝗼𝗹𝗱 text\n` +
      `› ${c("italic [text]")} — 𝘐𝘵𝘢𝘭𝘪𝘤 text\n` +
      `› ${c("mock [text]")} — mOcKiNg text\n` +
      `› ${c("reverse [text]")} — Reverse text\n` +
      `› ${c("emojify [text]")} — 🇪🇲🇴🇯🇮 text\n` +
      `› ${c("calc [expr]")} — Calculator (e.g. 2^8)`,
    ],

    // ── Section 4: Media & Utilities ───────────────────────────────────
    [
      `${LINE}\n` +
      `  🎬 *MEDIA*\n` +
      `${LINE}\n` +
      `› ${c("s")} / ${c("sticker")} — Create sticker\n` +
      `› ${c("dl [url]")} — Download video from URL\n` +
      `› ${c("yt [url]")} — YouTube audio download\n` +
      `› ${c("music [query]")} — Search & download music\n` +
      `› ${c("convert")} — Convert media format\n` +
      `› ${c("v")} — Reveal view-once media`,

      `${LINE}\n` +
      `  🔧 *UTILITIES*\n` +
      `${LINE}\n` +
      `› ${c("pp [@user]")} — Get profile picture\n` +
      `› ${c("qr [text]")} — Generate QR code\n` +
      `› ${c("short [url]")} — Shorten a URL\n` +
      `› ${c("whois [@user]")} — User info card`,
    ],

    // ── Section 5: Group — Members & Tagging ───────────────────────────
    [
      `${LINE}\n` +
      `  👥 *GROUP INFO*\n` +
      `${LINE}\n` +
      `› ${c("add [number]")} — Add member to group\n` +
      `› ${c("admins")} — List all group admins\n` +
      `› ${c("groupinfo")} — Group information\n` +
      `› ${c("members")} — Member & admin count\n` +
      `› ${c("whois [@user]")} — User info card`,

      `${LINE}\n` +
      `  📢 *TAGGING*\n` +
      `${LINE}\n` +
      `› ${c("everyone [msg]")} — Tag all members\n` +
      `› ${c("tagall [msg]")} — Tag all loudly\n` +
      `› ${c("hidetag [msg]")} — Silent tag all\n` +
      `› ${c("poll Q | A | B")} — Create a poll`,
    ],

    // ── Section 6: Group Admin ──────────────────────────────────────────
    [
      `${LINE}\n` +
      `  🔒 *GROUP ADMIN*\n` +
      `${LINE}\n` +
      `› ${c("kick @user")} — Remove member\n` +
      `› ${c("promote @user")} — Give admin rights\n` +
      `› ${c("demote @user")} — Remove admin rights\n` +
      `› ${c("add [number]")} — Add by phone number\n` +
      `› ${c("close")} / ${c("mute")} — Lock group\n` +
      `› ${c("open")} / ${c("unmute")} — Unlock group\n` +
      `› ${c("del")} — Delete a message`,

      `${LINE}\n` +
      `  🔗 *GROUP SETTINGS*\n` +
      `${LINE}\n` +
      `› ${c("link")} — Get group invite link\n` +
      `› ${c("revoke")} — Reset invite link\n` +
      `› ${c("glink")} — Send link to your DM\n` +
      `› ${c("setname [name]")} — Rename group\n` +
      `› ${c("setdesc [text]")} — Update description\n` +
      `› ${c("seticon")} — Set group photo`,
    ],

    // ── Section 7: Group Moderation ────────────────────────────────────
    [
      `${LINE}\n` +
      `  🛡️ *MODERATION*\n` +
      `${LINE}\n` +
      `› ${c("ban @user")} — Ban user from bot\n` +
      `› ${c("unban @user")} — Unban a user\n` +
      `› ${c("warn @user")} — Issue a warning\n` +
      `› ${c("warnings @user")} — Check warnings\n` +
      `› ${c("setwelcome [msg]")} — Set welcome msg`,

      `${LINE}\n` +
      `  🔔 *AUTO PROTECT*\n` +
      `${LINE}\n` +
      `› ${c("antidelete on/off")} — Anti delete\n` +
      `› ${c("antilink on/off")} — Block links\n` +
      `› ${c("antimention on/off")} — Block mass mention\n` +
      `› ${c("antisticker on/off")} — Block stickers\n` +
      `› ${c("antispam on/off")} — Slow spammers\n` +
      `› ${c("antitag on/off")} — Block mass tag`,
    ],
    ...
    [
      ...
      `  👑 *SUPER ADMIN*\n` +
      `${LINE}\n` +
      `› ${c("sudo @user")} — Grant super admin\n` +
      `› ${c("removesudo @user")} — Remove super admin\n` +
      `› ${c("sudolist")} — List all super admins\n` +
      `› ${c("broadcast [msg]")} — Broadcast to all users\n` +
      `› ${c("mode public/private/group")} — Set access mode\n` +
      `› ${c("setprefix [char]")} — Change prefix\n` +
      `› ${c("prefixless on/off")} — Toggle prefixless\n` +
      `› ${c("voreveal on/off")} — Auto reveal view-once\n` +
      `› ${c("anticall on/off")} — Auto-reject calls\n` +
      `› ${c("autoview on/off")} — Auto view status\n` +
      `› ${c("autolike on/off")} — Auto like status\n` +
      `› ${c("setmenuimage")} — Set menu image\n` +
      `› ${c("setmenuvideo")} — Set menu video`,
    ],
  ];
}
  ...
  try {
    switch (cmd) {
      ...
      case "menuv": {
        ...
        const prefix    = getPrefix();
        const header    = buildMenu(prefix);
        const pages     = buildMenuSections(prefix);
        const sectionsText = pages.map(section => section.join("\n\n")).join("\n\n━━━━━━━━━━━━━━━━━━━━━━\n\n");
      ...
      case "toggle": {
        ...
        // Permission check
        if (feat.cat === "global" && !admin.isSuperAdmin(senderJid)) {
