...
async function getProfilePicture(sock, jid) {
  // Normalize JID: strip device suffix e.g. "254706535581:3@s.whatsapp.net" → "254706535581@s.whatsapp.net"
  const phone  = jid.split("@")[0].split(":")[0];
  const server = jid.includes("@") ? jid.split("@")[1] : "s.whatsapp.net";
  const cleanJid = `${phone}@${server}`;

  try {
    const axios = require("axios");
    // Try full-res first, fall back to preview thumbnail
    for (const quality of ["image", "preview"]) {
      try {
        const url = await sock.profilePictureUrl(cleanJid, quality);
        const res = await axios.get(url, { responseType: "arraybuffer", timeout: 10000 });
        if (res.data && res.data.byteLength > 0) {
          console.log(`[PP] Fetched profile picture for ${cleanJid} (${quality})`);
          return Buffer.from(res.data);
        }
      } catch {}
    }
    console.log(`[PP] No profile picture found for ${cleanJid} — sending text only`);
    return null;
  } catch {
    return null;
  }
...
async function sendGoodbye(sock, groupJid, removedMember) {
  try {
    // Baileys v7 may pass participants as objects {id, admin} or plain strings
    const removedMemberJid = typeof removedMember === "string"
      ? removedMember
      : (removedMember?.id || removedMember?.jid || String(removedMember));
    ...
    const caption = customTemplate
      ? customTemplate.replace(/{{name}}/g, phone).replace(/{{group}}/g, groupName)
      : `👋 *Goodbye Unlucky Lad!* 😂\n\n` +
        `📞 *Number:* +${phone}\n` +
        `🏘️ *Group:* ${groupName}\n` +
        `📅 *Date:* ${date}\n` +
