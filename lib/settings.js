...
const DEFAULTS = {
  ...
  antiDeleteMode:   "off",             // "off" | "private" | "group" | "chat" | "both" | "all" | "status"
  antiEditMode:     "off",             // "off" | "private" | "chat" | "both" | "all"
...
function initSettings() {
  db.update("settings", DEFAULTS, (data) => {
    // Add any missing keys with their defaults
    for (const [k, v] of Object.entries(DEFAULTS)) {
      if (!(k in data)) data[k] = v;
    }
    // One-time migration: voReveal default changed from false → true.
    // Only runs once (_voRevealMigrated flag). After this the user's own
    // .viewonce on/off commands control the setting as normal.
    if (!data._voRevealMigrated) {
      data.voReveal = true;
      data._voRevealMigrated = true;
    }
  });
}
...
function getMenuVideo() {
  ...
  // Fallback: bundled default video (data/ first, then assets/)
  for (const rel of ["data/menu_video.mp4", "assets/menu.mp4"]) {
...
function getMenuImage() {
  ...
  if (fromDb) return fromDb;
  // Fallback: bundled default image (data/ first, then assets/)
  for (const rel of ["data/menu_image.jpg", "assets/menu_image.jpg"]) {
    const p = path.join(process.cwd(), rel);
    if (fs.existsSync(p)) return fs.readFileSync(p);
  }
  return null;
...
function getMenuSong() {
  ...
  if (fromDb) return fromDb;
  // Fallback: bundled default song (data/ first, then assets/)
  for (const rel of ["data/menu_song.mp3", "assets/menu.mp3"]) {
    const p = path.join(process.cwd(), rel);
    if (fs.existsSync(p)) return fs.readFileSync(p);
  }
  return null;
...
function formatSettingsMessage() {
  ...
  return (
    ...
    `💾 *Save Status:* Command-only (.savestatus)\n` +
    ...
    `👁 *View-Once Auto-Reveal:* ${on(s.voReveal)}\n\n` +
