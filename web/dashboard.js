...
.bnav-item.bn-setup.active { color:#f59e0b; }
.bnav-item.bn-setup.active::before { background:#f59e0b; box-shadow:0 0 8px #f59e0b; }
.bnav-item.bn-add.active { color:var(--green); }
.bnav-item.bn-add.active::before { background:var(--green); box-shadow:0 0 8px var(--green); }
...
@media(max-width:768px){
  ...
  .card{padding:16px}
  .section,.form-section{padding:16px}
...
<nav class="bottom-nav">
  <div class="bottom-nav-inner">
    <a class="bnav-item ${activeTab==="overview"?"active":""}" href="/dashboard?tab=overview">
      <span class="bnav-icon">📊</span>
      <span class="bnav-label">Overview</span>
    </a>
    <a class="bnav-item ${activeTab==="session"?"active":""}" href="/dashboard?tab=session">
      <span class="bnav-icon">🔑</span>
      <span class="bnav-label">Session</span>
    </a>
    <a class="bnav-item bn-setup ${activeTab==="setup"?"active":""}" href="/dashboard?tab=setup">
      <span class="bnav-icon">⚙️</span>
      <span class="bnav-label">Setup</span>
    </a>
    <a class="bnav-item bn-add ${activeTab==="add"?"active":""}" href="/dashboard?tab=add">
      <span class="bnav-icon">➕</span>
...
<div id="tabSetup" style="display:${activeTab==="setup"?"block":"none"}">
  ...
  <div class="form-section">
    ...
    <div style="display:flex;gap:10px;flex-wrap:wrap">
      ...
      <button class="btn btn-cyan" onclick="copyEnvBlockSetup()">📄 Copy All Vars as .env</button>
    </div>
...
<div id="tabAdd" style="display:${activeTab==="add"?"block":"none"}">
  ...
  <div class="form-section">
    ...
    <div style="margin-top:24px;display:flex;gap:10px;flex-wrap:wrap;align-items:center">
      ...
      <button class="btn btn-purple" onclick="copyEnvBlockAdd()">📄 Copy All Vars as .env</button>
...
function _buildEnvBlock(vars) {
  const lines = [
    '# ── NEXUS-MD Environment Variables ──────────────────────────────',
    '# Generated by the NEXUS-MD dashboard. Paste into .env or any',
    '# platform\'s environment settings (Render, Railway, Replit, VPS).',
    '# ─────────────────────────────────────────────────────────────────',
    '',
  ];
  ...
  lines.push('');
  lines.push('# DATABASE_URL is auto-provided by Heroku Postgres — leave blank on Heroku.');
  lines.push('# DATABASE_URL=');
  return lines.join('\\n');
}

function copyEnvBlockSetup() {
  const phone     = (document.getElementById('setupPhone').value||'').replace(/\\D/g,'').trim();
  const sessionId = (document.getElementById('setupSessionId').value||'').trim();
  const botname   = (document.getElementById('setupBotname').value||'NEXUS-MD').trim();
  const badword   = (document.getElementById('setupBadword')?.value||'fuck,pussy,slut,bitch,cock,stupid').trim();
  const menuType  = (document.getElementById('setupMenuType')?.value||'VIDEO');
  const out       = document.getElementById('herokuFillOutput');

  if (!phone && !sessionId) {
    out.innerHTML = '<span style="color:var(--gold)">⚠️ Fill in your phone number and/or session ID above first.</span>';
    return;
  }

  const vars = {};
  if (sessionId) { vars['SESSION_ID'] = sessionId; vars['SESSION'] = sessionId; }
  if (phone)     vars['ADMIN_NUMBERS'] = phone;
  vars['BOTNAME']      = botname;
  vars['BAD_WORD']     = badword;
  vars['MENU_TYPE']    = menuType;
  vars['PAIR_SITE_URL']= 'https://nexus-session-76ah.onrender.com';
  vars['NODE_ENV']     = 'production';

  const block = _buildEnvBlock(vars);

  navigator.clipboard.writeText(block).then(() => {
    toast('✅ .env block copied to clipboard!');
    out.innerHTML = \`
      <div style="background:rgba(16,185,129,0.07);border:1px solid rgba(16,185,129,0.25);border-radius:10px;padding:14px 16px;margin-top:4px">
        <p style="font-size:0.78rem;color:var(--green);font-weight:600;margin:0 0 10px">✅ Copied! Paste into your .env file or platform settings:</p>
        <pre style="font-family:'JetBrains Mono',monospace;font-size:0.72rem;color:var(--cyan);background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:8px;padding:12px;white-space:pre-wrap;word-break:break-all;margin:0;max-height:280px;overflow-y:auto">\${block.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
      </div>\`;
  }).catch(() => {
    out.innerHTML = \`
      <div style="background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:10px;padding:14px 16px">
        <p style="font-size:0.78rem;color:var(--muted);margin:0 0 10px">Select all and copy manually:</p>
        <pre style="font-family:'JetBrains Mono',monospace;font-size:0.72rem;color:var(--cyan);background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:8px;padding:12px;white-space:pre-wrap;word-break:break-all;margin:0;max-height:280px;overflow-y:auto;cursor:text;user-select:all">\${block.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
      </div>\`;
    toast('Select the text above and copy manually', '#d29922');
  });
}

function copyEnvBlockAdd() {
  const phone     = (document.getElementById('addAdminNumbers').value||'').replace(/\\D/g,'').trim();
  ...
  const badword   = (document.getElementById('addBadword').value||'fuck,pussy,slut,bitch,cock,stupid').trim();
  const menuType  = (document.getElementById('addMenuType').value||'VIDEO');
  const pairSite  = (document.getElementById('addPairSite').value||'https://nexus-session-76ah.onrender.com').trim();
  const resultEl  = document.getElementById('addResult');

  if (!phone && !sessionId) {
    resultEl.innerHTML = '<span style="color:var(--gold)">⚠️ Fill in your phone number and/or session ID first.</span>';
    return;
  }
  ...
  navigator.clipboard.writeText(block).then(() => {
    toast('✅ .env block copied to clipboard!');
    resultEl.innerHTML = \`
      <div style="background:rgba(16,185,129,0.07);border:1px solid rgba(16,185,129,0.25);border-radius:10px;padding:14px 16px">
        <p style="font-size:0.78rem;color:var(--green);font-weight:600;margin:0 0 10px">✅ Copied! Paste into your .env file or platform settings (Render, Railway, Replit, VPS):</p>
        <pre style="font-family:'JetBrains Mono',monospace;font-size:0.72rem;color:var(--cyan);background:rgba(0,0,0,0.4);border:1px solid var(--border);border-radius:8px;padding:12px;white-space:pre-wrap;word-break:break-all;margin:0;max-height:280px;overflow-y:auto">\${block.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')}</pre>
      </div>\`;
  }).catch(() => {
    resultEl.innerHTML = \`
